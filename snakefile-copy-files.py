...
# sampleSheetPath = str('master-samplesheet-rnaseq.csv')
# sampleSheetPath = str('master-samplesheet-cutnrun.csv')
sampleSheetPath = str('master-samplesheet-crisprGFP-HP1a.csv')
...
rule all:
	input:
		expand("../astutzman/{fastq}", fastq=fastqList)
...
rule copyFiles:
	...
	output:
		expand('../astutzman/{fastq}', fastq = list(sampleDF.fastqName))
	...
	run:
		for htsf in list(sampleDF.htsfFile):
			...
			outFile = '../astutzman_cutnrun/{fastq}'.format(fastq = outFileBase)
			#print('THERE')
			#print(outFile)
