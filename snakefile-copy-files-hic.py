...
rule all:
	input:
		expand("../astutzman_HiC/{fastq}.fastq.gz", fastq=fastqList)
		#expand("../astutzman_HiC/{tech}_{readNum}.fastq.gz", tech=techList, readNum=readNumList),
...
rule copyFiles:
	...
	output:
		expand('../astutzman_HiC/{fastq}.fastq.gz', fastq = list(sampleDF.fastqName))
	...
	run:
		for htsf in list(sampleDF.htsfFile):
			...
			outFile = '../astutzman_HiC/{fastq}.fastq.gz'.format(fastq = outFileBase)
			#print('THERE')
			#print(outFile)
