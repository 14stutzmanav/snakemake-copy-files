...
rule all:
	input:
		expand("../astutzman_rnaseq/{fastq}.fastq.gz", fastq=fastqList)
...
rule copyFiles:
	...
	output:
		expand('../astutzman_rnaseq/{fastq}.fastq.gz', fastq = list(sampleDF.fastqName))
	...
	run:
		for htsf in list(sampleDF.htsfFile):
			...
			outFile = '../astutzman_rnaseq/{fastq}.fastq.gz'.format(fastq = outFileBase)
			#print('THERE')
			#print(outFile)
